...
-- /dump BuffToSoothe("player")
function BuffToSoothe(unit)
  for i = 1, #enragelist do
  local BuffToStealName = _G.GetSpellInfo(enragelist[i])
   if NeP.DSL:Get("buff.any")(unit, BuffToStealName) then
    return true
   end
  end
...
--Enemies with buffs that needs to be soothed
-- /dump NeP.FakeUnits:Filter("enemysoothebuff")
NeP.FakeUnits:Add("enemysoothebuff", function()
  for _, Obj in pairs(NeP.OM:Get("Enemy", true)) do
   if Obj.distance < 40
    and _G.UnitCanAttack(Obj.key, "player")
    and BuffToSoothe(Obj.key)
    and not NeP.DSL:Get("immune_all")(Obj.key)
    and not NeP.DSL:Get("immune_spell")(Obj.key) then
     return Obj.key
   end
