...
local exeOnLoad = function()
 	...
 	print("|c0000FA9A ----------------------------------------------------------------------|r")
    print("|c0000FA9A --- |r"..ColorClass.." MAGE - Fire|r")
...
local Keybinds = {
    ...
    {"Polymorph", "inRange.spell & canAttack & !player.moving & debuff(Polymorph).duration <= 2 & !player.lastcast & !focus.exists & !state(root) & !state(stun) & !state(fear) & !state(disorient) & !state(incapacitate) & !state(charm) & {keybind(alt) & UI(list1)==3 || keybind(shift) & UI(list1)==1 || keybind(control) & UI(list1)==2}", "target"},
    {"Polymorph", "inRange.spell & canAttack & !player.moving & debuff(Polymorph).duration <= 2 & !player.lastcast & focus.exists & !state(root) & !state(stun) & !state(fear) & !state(disorient) & !state(incapacitate) & !state(charm) & {keybind(alt) & UI(list1)==3 || keybind(shift) & UI(list1)==1 || keybind(control) & UI(list1)==2}", "focus"},
	{"Meteor", "UI(list4)==11 & target.canAttack & player.combat & target.range <= 38.5 & {keybind(alt) & UI(list3)==9 || keybind(shift) & UI(list3)==7 || keybind(control) & UI(list3)==8}", "target.ground"},
...
local PreCombat = { 
	...
	{"Fireball", "inRange.spell & canAttack & infront & !player.moving", "target"},
...
local Interrupts = {
	...
	{"Arcane Torrent", "interruptAt(1) & fixRange <= 7.5 & !player.casting(Barrage)", "target"},
...
local inCombat = {
	...
	{Interrupts, "toggle(interrupts) & !target.casting(Vengeful Wail) & !target.casting(Runic Empowerment) & !player.buff(Invisibility)"},
	...
	{Cooldowns, "toggle(cooldowns) & target.canAttack & !player.casting(Rune of Power)"},
    {Combat, "target.canAttack & !player.casting(Rune of Power)"},
...
NeP.CR:Add(63, {
    name = "["..ColorClass.."Kleei|r]"..ColorClass.." MAGE - Fire",
    ic = inCombat,
    ooc = outCombat,
    gui = GUI,
