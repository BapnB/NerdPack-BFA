...
local GUI = {
	...
	{type = "text", text = "", align = "center"}, --------------------------------------
	{type = "dropdown", text = "Use Death Grip:|c0000FA9A on mouseover, target", key = "list2", width = 100, list = {
...
local exeOnLoad = function()
 	...
 	print("|c0000FA9A ------------------------------------------------------------------------------|r")
 	print("|c0000FA9A --- |r"..ColorClass.."DEATH KNIGHT - Frost|r")
...
local Keybinds = {
    ...
    {"Chains of Ice", "inRange.spell & !debuff.any & canAttack & infront & !state(stun) & !state(disorient) & {keybind(alt) & UI(list1)==3 || keybind(shift) & UI(list1)==1 || keybind(control) & UI(list1)==2}", {"mouseover", "target"}},
    {"Death Grip", "inRange.spell & canAttack & infront & distance > 8 & {keybind(alt) & UI(list2)==6 || keybind(shift) & UI(list2)==4 || keybind(control) & UI(list2)==5}", {"mouseover", "target"}},
...
local Combat = {
	...
	{"Remorseless Winter", "target.inRange(Frost Strike).spell & target.deathin > 8", "player"}, --, "artifact(Frozen Soul).enabled"
 	{"Frost Strike", "inRange.spell & !player.buff(Breath of Sindragosa) & {{talent(1,2) & {player.buff(Icy Talons).count < 3 || player.buff(Icy Talons).duration <= 2.2}} || player.buff(Obliteration) & !player.buff(Killing Machine)}", "target"},
    ...
    {"Obliterate", "inRange.spell", "target"},
	{"Frost Strike", "inRange.spell & !player.buff(Breath of Sindragosa)", "target"},
...
NeP.CR:Add(251, {
	name = "["..ColorClass.."Kleei|r]"..ColorClass.." DEATH KNIGHT - Frost",
	ic = inCombat,
	ooc = outCombat,
	gui = GUI,
