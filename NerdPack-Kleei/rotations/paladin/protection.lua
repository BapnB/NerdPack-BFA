...
local exeOnLoad = function()
 	...
 	print("|c0000FA9A ----------------------------------------------------------------------|r")
    print("|c0000FA9A --- |r"..ColorClass.." PALADIN - Protection|r")
...
local pvp = {
    ...
    {"!Gladiator's Medallion", "UI(medal) & target.canAttack & target.player & {player.state(stun) || player.state(fear) || player.state(disorient) || player.state(charm)}", "player"},
    {"!Blessing of Freedom", "UI(bof_key) & {player.state(snare) || player.state(root)}", "player"},
...
local Keybinds = {
    ...
    {"!Divine Shield", "player.combat & target.canAttack & {keybind(alt) & UI(list2)==6 || keybind(shift) & UI(list2)==4 || keybind(control) & UI(list2)==5}", "player"},
    {"!Flash of Light", "health <= 95 & !moving & {keybind(alt) & UI(list3)==9 || keybind(shift) & UI(list3)==7 || keybind(control) & UI(list3)==8}", "player"},
	{"Hammer of Justice", "inRange.spell & canAttack & {keybind(alt) & UI(list1)==3 || keybind(shift) & UI(list1)==1 || keybind(control) & UI(list1)==2 || UI(stun) & target.player & !target.state(stun) & !target.state(disorient) & !target.state(incapacitate)}", "target"},
...
local Interrupts = {
    ...
    {"Rebuke", "interruptAt(65)", "enemies"},
    --{"Hammer of Justice"},
	{"Arcane Torrent", "interruptAt(65) & canAttack & inRange(Shield of the Righteous).spell", "target"},

}

local Survival ={

    {"&Lay on Hands", "player.health <= UI(loh_key_spin) & UI(loh_key_check) & !player.lastcast(Divine Shield) & !player.buff(Divine Shield)", "player"},
    {"&Divine Shield", "player.health <= UI(ds_key_spin) & UI(ds_key_check) & !player.lastcast(Lay on Hands)", "player"},
    {"&Light of the Protector", "inRange.spell & player.health <= UI(lotp_key_spin) & UI(lotp_key_check)", "lowest"},
    {"!Hand of the Protector", "player.health <= UI(hotp_key_spin) & UI(hotp_key_check)", "player"},
    --{"!Hand of the Protector", "health <= 40 & player.health >= 90", "lowest"},
...
local outCombat = {
    ...
    {"!Flash of Light", "health <= 80 & !moving", "player"},

}

NeP.CR:Add(66, {
    name = "["..ColorClass.."Kleei|r]"..ColorClass.." Paladin - Protection",
    ic = inCombat,
    ooc = outCombat,
    gui = GUI,
