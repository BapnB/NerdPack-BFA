...
local exeOnLoad = function()
    ...
    NeP.Interface:AddToggle({key = "prepot_key", icon = "Interface\\Icons\\trade_alchemy_dpotion_a28", name = "Use Potion/Flask", text = "|rThis will auto use |c0000BFFF[Flask of the Seventh Demon]|r and |c0000BFFF[Potion of Prolonged Power]\n|rCooldowns Toggle should be ON."})
...
local Cooldowns ={
    ...
    {"Blood Fury", "target.inRange(Backstab).spell", "player"},
    {"Berserking", "target.inRange(Backstab).spell", "player"},
...
local Combat = {
	...
	--Potion and Flask
    {"#Flask of the Seventh Demon", "toggle(cooldowns) & !buff(Flask of the Seventh Demon) & target.inRange(Backstab).spell & energy >= 34 & toggle(prepot_key)", "player"},
    {"#Potion of Prolonged Power", "toggle(cooldowns) & target.inRange(Backstab).spell & energy >= 34 & toggle(prepot_key)", "player"},
	...
	--Shadow Dance
    {"Shadow Dance", "target.inRange(Backstab).spell & !buff & !buff(Subterfuge) & !IsStealthed & combopoints.deficit > 2 & energy >= 34 & {!equipped(The First of the Dead) || equipped(The First of the Dead) & !lastcast(Symbols of Death).succeed} & {player.area(8).enemies < 3 || !toggle(aoe)} & {target.health.actual >= player.health.max * 0.4 || target.isdummy || target.player} & {spell(Shadow Dance).charges == 2 & {lastcast(Nightblade).succeed || lastcast(Eviscerate).succeed} || spell(Shadow Dance).charges == 2 & player.combat.time > 10 || spell(Shadow Dance).charges == 1 & shadow_dance_timing}", "player"},
	...
	--Build Combo Points
    {"Shuriken Storm", "toggle(aoe) & combopoints.deficit > 0 & !lastcast(Vanish).succeed & !buff(Vanish) & {player.area(8).enemies >= 3 || player.buff(The Dreadlord's Deceit).count >= 30}", "player"},
    {Shadowstrike, "toggle(dfa_strike) & target.inRange(Shadowstrike).spell & player.combat & !target.player & {player.buff(Stealth) || player.buff(Shadow Dance) || player.buff(Subterfuge)} & combopoints.deficit > 0 & !target.name(Krosus)"},
    {Shadowstrike, "target.inRange(Backstab).spell & player.combat & !target.player & {player.buff(Stealth) || player.buff(Shadow Dance) || player.buff(Subterfuge)} & combopoints.deficit > 0 & target.name(Krosus)"},
    {"Shadowstrike", "toggle(dfa_strike) & {target.inRange(Backstab).spell & !player.combat || target.player & target.inRange(Shadowstrike).spell & !IsStealthed} & combopoints.deficit > 0 & !name(Krosus)", "target"},
    {"Backstab", "inRange.spell & combopoints.deficit > 0 & {!IsStealthed || !toggle(dfa_strike)}", "target"},
    {"Gloomblade", "inRange.spell & combopoints.deficit > 0 & {!IsStealthed || !toggle(dfa_strike)}", "target"},
...
local outCombat = {
    ...
    {pvp},
    {"/stopattack", "buff(Vanish) & target.player & target.canAttack", "player"},
    {"Stealth", "UI(stealth_key) & !state(dot) & !IsStealthed & timeout(stealth_1, 0.3)", "player"},
