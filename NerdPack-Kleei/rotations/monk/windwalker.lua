...
local pvp = {
    ...
    {"Disable", "inRange.spell & player & !debuff(Disable) & !target.buff(Blessing of Freedom)", "target"},
...
local Precombat = { 
    ...
    --{"Effuse", "health < 99 & !player.debuff(Control the Mists) & pvp.area", "player"},

}

local Interrupts = {

	{"!Spear Hand Strike", "interruptAt(63) & inRange.spell", "target"},
	{"!Spear Hand Strike", "interruptAt(63) & inRange.spell", "enemies"},
	{"!Arcane Torrent", "interruptAt(60) & inRange(Tiger Palm).spell", "target"},
...
local Survival = {
	...
	--{"Effuse", "health < 80 & !player.debuff(Control the Mists) & pvp.area", "player"},
}
...
local Cooldowns = {
	...
	{"Touch of Death", "inRange.spell & UI(ToD_key) & !lastcast(Fists of Fury).succeed & {target.health.actual >= player.health.max * 4 || target.isdummy || target.player}", "target"},
...
local Combat = {
	...
	{"Whirling Dragon Punch", "area(7).enemies >= 1", "player"},
...
local inCombat = {
	...
	{Keybinds},
	{Interrupts, "toggle(interrupts) & target.canAttack & !player.lastcast(Fists of Fury)"},
