...
local Corruption, CorruptionResistance = GetCorruption(), GetCorruptionResistance()
local TotalCorruption = Corruption - CorruptionResistance

--/dump NeP.DSL.Parse("corruption", "", "")
NeP.DSL:Register("corruption", function()
...
local immunallbuff = { 642, 133093, 63148, 186265, 19263, 122464, 122465, 124280, 125174, 45438, 145533, 41590, 36911, 27619, 47585 }
...
NeP.DSL:Register("immune_all",function(target)
  ...
  local BuffName = _G.GetSpellInfo(immunallbuff[i])
   if NeP.DSL:Get("buff.any")(target, BuffName) or NeP.DSL:Get("buff.any")(target, _G.GetSpellInfo(122470)) and NeP.DSL:Get("debuff")("player", _G.GetSpellInfo(122470)) then
    return true 
   end
  end
...
local noroot = {
	...
	48707,  	-- AMS                  	   		   -- Death Knight

}

--/dump NeP.DSL.Parse("immune_root", "", "")
NeP.DSL:Register("immune_root", function(target)
  for i = 1, #noroot do
  local SpellNameRoot = _G.GetSpellInfo(noroot[i])
   if NeP.DSL:Get("buff.any")(target, SpellNameRoot)  then
    return true
   end
  end
end)
...
--/dump NeP.DSL.Parse("isChanneling", "", "")
NeP.DSL:Register("isChanneling", function(unit)
    local name, _, _, _, _, _, notInterruptible = _G.UnitChannelInfo(unit)
...
local intcast = {
   ...
   206931,  -- Blooddrinker                  	   -- Death Knight

   198013,  -- Eye Beam                 		   -- Demon Hunter
   258925,	-- Fel Barrage						   -- Demon Hunter

   157982,  -- Tranquility                    	   -- Druid //Database>Spells>Uncategorized???
   
   5143,	-- Arcane Missiles					   -- Mage
   
   191837,  -- Essence Font                  	   -- Monk
   115175,  -- Soothing Mist                 	   -- Monk
   
   47540,  	-- Penance						   	   -- Priest
   
   204437,  -- Lightning Lasso      			   -- Shaman //Database>Spells>Uncategorized???
   
   234153,	-- Drain Life						   -- Warlock


}

--/dump NeP.DSL.Parse("pvp.interrupts", "", "")
NeP.DSL:Register("pvp.interrupts", function(unit)
  for i = 1, #intcast do
  local SpellName = _G.GetSpellInfo(intcast[i])
   if NeP.DSL:Get("casting")(unit, SpellName)  then
    return true
   end
  end
end)
...
--/dump NeP.DSL.Parse("target.distance", "", "")
...
--/dump NeP.DSL.Parse("thorns.time", "", "")
--/dump NeP.DSL.Parse("target.inMelee", "", "")
--/dump NeP.DSL.Parse("target.alive", "", "")
NeP.DSL:Register("thorns.time", function(unit)
	if UnitIsUnit("targettarget", "player") and
		NeP.DSL:Get("inMelee")("target", "player") then return true
	end 
end)
--------------------------------------------------------------------------------
------------------------------ Azerite Traits ----------------------------------
...
-- Check to use Thrash on ST for ferals
NeP.DSL:Register("ThrashST", function()
	local ThrSt =
		-- Twisted Claws
		C_AzeriteEmpoweredItem.IsPowerSelected(ItemLocation:CreateFromEquipmentSlot(1), 241) or 
		C_AzeriteEmpoweredItem.IsPowerSelected(ItemLocation:CreateFromEquipmentSlot(3), 241) or 
		C_AzeriteEmpoweredItem.IsPowerSelected(ItemLocation:CreateFromEquipmentSlot(5), 241) or
		-- Wild Fleshrending
		C_AzeriteEmpoweredItem.IsPowerSelected(ItemLocation:CreateFromEquipmentSlot(1), 359) or 
		C_AzeriteEmpoweredItem.IsPowerSelected(ItemLocation:CreateFromEquipmentSlot(3), 359) or 
		C_AzeriteEmpoweredItem.IsPowerSelected(ItemLocation:CreateFromEquipmentSlot(5), 359)
	return ThrSt 
end)

-- Prevent to stun NPC's by their name 
-- Ex: {"StunSpell", "!hasName", "target"},
local nameslist = {
	...
	-- Shrine of the Storm V
	["Colossal Tentacle"]   	 	= true,
...
NeP.DSL:Register("hasName", function(unit)
    return nameslist[_G.UnitName(unit)] ~= nil
end)


-- Enrage Removal list for ferals
local enragelist = {
...
--/dump NeP.DSL.Parse("tosoothe", "", "")
NeP.DSL:Register("tosoothe", function (unit)
  for i = 1, #enragelist do
  local buffName = _G.GetSpellInfo(enragelist[i])
   if NeP.DSL:Get("buff.any")(unit, buffName) then return true
   end
  end
end)

--[[NeP.DSL:Register("tosoothe", function(unit)
    return enragelist[_G.GetSpellInfo(unit)] ~= nil
end)]]--
...
--/dump NeP.DSL.Parse("target.swing", "", "")
--/dump NeP.DSL.Parse("swing", "", "")
NeP.DSL:Register("swings", function()



		local timestamp, subevent, _, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags = CombatLogGetCurrentEventInfo("player")
		local spellId, spellName, spellSchool
		local amount, overkill, school, resisted, blocked, absorbed, critical, glancing, crushing, isOffHand

		if subevent == "SWING_DAMAGE" and
			amount > 0 then return true
			
		end

end)

NeP.Listener:Add('swing', 'SWING_DAMAGE', function(amount, _, _, _, _, _, _, _, _, _)
	if amount > 1 then 
		return true
	end
end)
NeP.Listener:Add('swing', 'COMBAT_LOG_EVENT_UNFILTERED', function()
	local playerGUID = NeP.DSL:Get('guid')('player')
	local damageEvents = {
    SWING_DAMAGE = true,
    SPELL_DAMAGE = true,
	}
...
--/dump attackedbymelee
attackedbymelee = false

NeP.Listener:Add("swing", "COMBAT_LOG_EVENT_UNFILTERED", function()
  
  local _, subevent, _, sourceGUID, _, _, _, destGUID, _, _, _, j = CombatLogGetCurrentEventInfo()

  if subevent == "SWING_DAMAGE" then
    if destGUID == _G.UnitGUID("player") or destGUID == _G.UnitGUID("player") and sourceGUID == _G.UnitGUID("player") then
		print("blablabla")
		attackedbymelee = true
    elseif destGUID ~= _G.UnitGUID("player") then
		attackedbymelee = false
	end
  end
end)

--/dump NeP.DSL.Parse("swings", "", "")
NeP.DSL:Register("swings", function()
	if attackedbymelee == true then
		return true
	end
end)]]--
