...
--/dump NeP.DSL.Parse("ass", "", "")
NeP.DSL:Register("ass", function(target)
	local _, classId = UnitClassBase("target")
	local numSpec = GetNumSpecializationsForClassID(classId)
	local specId, _, _, _, _, _, aff = GetSpecializationInfoForClassID(classId)
	return specId
end)
